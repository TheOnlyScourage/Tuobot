...
async def register_match_unranked(ctx, m):
    ...
    for p in m.players:
        nick = get_nick(p)
        await db.update("qc_players", dict(nick=nick), keys=dict(channel_id=m.qc.id, user_id=p.id))
...
async def register_match_ranked(ctx, m):
    ...
    # Fill-in subs: {player2_id: (player1_id, team_idx)}
    # - Team won  → player2 gets normal win gains
    # - Team lost → player1 receives the loss penalty; player2 gets no change
    ...
    for p in m.players:
        ...
        # ── Fill-in sub handling ───────────────────────────────────────────────
        if p.id in fill_subs:
            player1_id, sub_team_idx = fill_subs[p.id]
            team_won = (m.winner is not None and m.winner == sub_team_idx)

            if not team_won:
                # Team lost or drew — apply rating penalty to player1, skip player2
                loss_change = after[p.id]['rating'] - before[p.id]['rating']

                p1 = await db.select_one(
                    ('rating', 'deviation', 'wins', 'losses', 'draws', 'streak'),
                    'qc_players',
                    where=dict(channel_id=m.qc.rating.channel_id, user_id=player1_id)
                )
                if p1 and p1.get('rating') is not None:
                    new_rating = max(0, p1['rating'] + loss_change)
                    if m.winner is None:  # draw
                        new_w, new_l, new_d = p1['wins'], p1['losses'], p1['draws'] + 1
                        new_streak = 0
                    else:  # loss
                        new_w, new_l, new_d = p1['wins'], p1['losses'] + 1, p1['draws']
                        new_streak = -1 if p1['streak'] >= 0 else p1['streak'] - 1

                    await db.update('qc_players', dict(
                        rating=new_rating,
                        wins=new_w, losses=new_l, draws=new_d,
                        streak=new_streak,
                        last_ranked_match_at=now,
                    ), keys=dict(channel_id=m.qc.rating.channel_id, user_id=player1_id))
                ...
                # Record player2 in match log only (no rating change)
                await db.insert('qc_player_matches', dict(
                    match_id=m.id, channel_id=m.qc.id,
                    user_id=p.id, nick=nick, team=team
                ))
                continue  # skip normal rating update for player2

        # ── Normal rating update ───────────────────────────────────────────────
        await db.update(
            "qc_players",
            dict(
                nick=nick,
